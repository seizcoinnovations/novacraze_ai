    ...
    <div class="row">
        ...
        <div class="col-xl-12 mb-3">
           <div class="float-right">
            @if ($groupUid)
            <a class="lw-btn btn btn-secondary" href="{{ route('vendor.contact.group.read.list_view') }}">{{ __tr('Back to Contact Groups') }}</a>
            @endif
            <button type="button" class="lw-btn btn btn-primary" data-toggle="modal" data-target="#lwAddNewContact"> {{ __tr('Create New Contact') }}</button>
            <button type="button" class="lw-btn btn btn-dark" data-toggle="modal" data-target="#lwExportDialog"> {{ __tr('Export Contacts') }}</button>
            <button type="button" class="lw-btn btn btn-dark" data-toggle="modal" data-target="#lwImportContactDialog"> {{ __tr('Import Contacts') }}</button>
           </div>
        </div>
        <!--/ button -->
        {{-- import contacts --}}
        <x-lw.modal id="lwImportContactDialog" :header="__tr('Import Contacts')" :hasForm="true" data-pre-callback="appFuncs.clearContainer">
                <x-lw.form id="lwImportContactDialogForm" :action="route('vendor.contact.write.import')" :data-callback-params="['modalId' => '#lwImportContactDialog', 'datatableId' => '#lwContactList']" data-callback="appFuncs.modelSuccessCallback">
                    ...
                    <div class="form-group ">
                        <input id="lwImportDocumentFilepond" type="file" data-allow-revert="true" data-label-idle="{{ __tr('Select XLSX File') }}" class="lw-file-uploader" data-instant-upload="true"
        ...
        {{-- /import contacts --}}
        {{-- export contacts --}}
        <x-lw.modal id="lwExportDialog" :header="__tr('Export Contacts')" :hasForm="true" data-pre-callback="appFuncs.clearContainer">
            <div class="lw-form-modal-body p-3">
                ...
                <p>{{  __tr('You can export blank excel file and fill with data according to column header and import it for updates.') }}</p>
                <a href="{{ route('vendor.contact.write.export') }}" data-method="post" class="btn btn-primary">{{ __('Export Blank Template') }}</a>
            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-secondary" data-dismiss="modal">{{ __tr('Close') }}</button>
            </div>
            ...
            <x-lw.form id="lwAddNewContactForm" :action="route('vendor.contact.write.create')"
                ...
                <div class="lw-form-modal-body">
                    ...
                    <x-lw.input-field type="number" id="lwPhoneNumberField" data-form-group-class=""
                        :label="__tr('Phone Number')" name="phone_number"
                    ...
                    <x-lw.checkbox id="lwAiBotEnable" checked name="enable_ai_bot" value="1" data-lw-plugin="lwSwitchery" :label="__tr('Enable AI Bot')" />
                    @endif
                </div>
                    <fieldset>
        ...
        <x-lw.modal id="lwDetailsContact" :header="__tr('Contact Details')">
            ...
            <script type="text/template" id="lwDetailsContactBody-template">
                ...
                <div>
                    <label class="small">{{ __tr('Phone Number') }}:</label>
                    <div class="lw-details-item">
                        <%- __tData.wa_id %>
                    </div>
                </div>
                ...
                <fieldset>
                    ...
                    <% _.forEach(__tData.groups, function(value, key) { %>
                        <span class="badge badge-light"><%- value.title %></span>
                    <%  } ); %>
                </fieldset>
                <fieldset>
                    <legend>{{  __tr('Other Information') }}</legend>
                    ...
                    <div class="mb-2">
                        ...
                        <div class="lw-details-item">
                            <%- _.get(_.find(__tData.custom_field_values, {'contact_custom_fields__id' : {{ $vendorContactCustomField->_id }} }), 'field_value') %>
                        </div>
                    </div>
                    @endforeach
                </fieldset>
        ...
        <x-lw.modal id="lwEditContact" :header="__tr('Edit Contact')" :hasForm="true">
            ...
            <x-lw.form id="lwEditContactForm" :action="route('vendor.contact.write.update')"
                ...
                <script type="text/template" id="lwEditContactBody-template">
                    ...
                    <input type="hidden" name="contactIdOrUid" value="<%- __tData._uid %>" />
                        ...
                        <!-- /Last_Name -->
                <!-- Country -->
                        <x-lw.input-field type="selectize" data-lw-plugin="lwSelectize" id="lwCountryEditField" data-form-group-class="" data-selected="<%- __tData.countries__id %>" :label="__tr('Country')" name="country"  required="true">
                    <x-slot name="selectOptions">
                        <option value=" ">{{ __tr('Country') }}</option>
                        @foreach(getCountryPhoneCodes() as $getCountryCode)
                        <option value="{{ $getCountryCode['_id'] }}">{{ $getCountryCode['name'] }}</option>
                ...
                <!-- /Country -->
                <!-- Phone_Number -->
               <x-lw.input-field class="disabled" disabled type="text" id="lwPhoneNumberEditField" data-form-group-class="" :label="__tr('Phone Number')" value="<%- __tData.wa_id %>" name="phone_number"  />
                <!-- /Phone_Number -->
                <!-- Language Code -->
            ...
            <!-- /Language Code -->
                ...
                <!-- /Email -->
                <x-lw.input-field type="selectize" data-lw-plugin="lwSelectize" id="lwEditSelectGroupsField" data-form-group-class="" data-selected="<%- __tData.existingGroupIds %>" :label="__tr('Groups')" name="contact_groups[]"
                multiple>
                <x-slot name="selectOptions">
                    <option value="">{{ __tr('Select Group') }}</option>
                    @foreach($vendorContactGroups as $vendorContactGroup)
        ...
        <div class="col-xl-12">
            <x-lw.datatable data-page-length="500" id="lwContactList" :url="route('vendor.contact.read.list', [
                ...
                <th data-name="phone_number">{{ __tr('Phone Number') }}</th>
...
@endif
 <a data-method="post" href="<%= __Utils.apiURL("{{ route('vendor.contact.write.delete', [ 'contactIdOrUid']) }}", {'contactIdOrUid': __tData._uid}) %>" class="btn btn-danger btn-sm lw-ajax-link-action-via-confirm" data-confirm="#lwDeleteContact-template" title="{{ __tr('Delete') }}" data-callback-params="{{ json_encode(['datatableId' => '#lwContactList']) }}" data-callback="appFuncs.modelSuccessCallback"><i class="fa fa-trash"></i> {{  __tr('Delete') }}</a>
        ...
        <script type="text/template" id="lwDeleteContact-template">
            ...
            <p>{{ __tr('Are you sure you want to delete this Contact?') }}</p>
    </script>
        <!-- /Contact delete template -->
    </div>
</div>
